...
export function buildJmpReplayPlan(program,validation={},origin='model_response'){
  if(!['model_response','synthetic_fixture'].includes(origin))throw Error('Invalid JMP plan origin');
  const state=validateJmpProgram(program,{...validation,final:true});
  ...
  const catalog=jmpApiCatalog(),used=new Set(program.operations.map(o=>o.apiCall.interfaceId));
  return {format:'jmp-api-replay-plan',version:'1.0',application:{name:'JMP',version:'19.1',language:'en-US'},
