...
export const JMP_API_CATALOG_VERSIONS=['jmp-jsl-18-v1',JMP_API_CATALOG_VERSION];
const definitions={
...
export function jmpApiCatalog(version='19.1'){
  if(!['18','19.1'].includes(version))throw Error('Unsupported JMP API catalog version '+version);
  const is18=version==='18';
  // Page numbers differ between major-version PDFs; never reuse a 19.x anchor.
  const source=uri=>is18?uri.replaceAll('/jmp19/','/jmp18/').replaceAll('/19.0/','/18.0/').replace(/#page=\d+$/,''):uri;
  return {version:`jmp-jsl-${version}-v1`,application:'JMP',applicationVersion:version,transport:'native_jsl',
    ...
    interfaces:Object.entries(definitions).map(([operationKind,d])=>({
      ...
      argumentsSchema:d.arguments,syntax:d.syntax,source:source(d.source),note:d.note??'',
