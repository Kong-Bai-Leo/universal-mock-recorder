...
export const VIVADO_BUDGET = Object.freeze({ maxRequestsPerRound: 2, maxImages: 12, maxOutputTokens: 6000, maxInputEvents: 60, maxPayloadBytes: 100000, maxRequestBytes: 8*1024*1024 });
export const digest = value => createHash("sha256").update(typeof value === "string" || Buffer.isBuffer(value) ? value : JSON.stringify(value)).digest("hex");
const processAllowed = e => ["vivado","vvgl"].includes(String(e.window?.processName ?? "").toLowerCase().replace(/\.exe$/,""));
...
export function groupVivadoEvents(events) {
...
export async function prepareVivadoEvidence(recording) {
  ...
  if (manifest.applicationProfile !== "vivado" || manifest.captureDeployment !== "same-windows-session") throw new Error("需要在 VIVADO 所在 VM 内采集的 VIVADO 录制，不能分析外层远程桌面输入。");
  ...
  if (!events.length) throw new Error("没有目标 VIVADO 进程输入；请检查录制器是否运行在 VM 内。");
  ...
  const fits = group => { const c=describe(group); return c.images.length<=VIVADO_BUDGET.maxImages && c.events.length<=VIVADO_BUDGET.maxInputEvents && Buffer.byteLength(JSON.stringify(c.events))<=VIVADO_BUDGET.maxPayloadBytes && c.images.reduce((n,x)=>n+Math.ceil(x.bytes/3)*4,0)<=VIVADO_BUDGET.maxRequestBytes-200000; };
  for (const group of groupVivadoEvents(events)) {
