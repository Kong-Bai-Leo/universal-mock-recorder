...
import {TOP_TRANSACTIONS_SCHEMA,VIVADO_STATE_RULES,validateTopTransactions,pendingTopTransactions} from './vivado-state-harness.mjs';
const str={type:'string'}, num={type:'number'}, arr=items=>({type:'array',items}), strs=arr(str);
const obj=properties=>({type:'object',additionalProperties:false,properties,required:Object.keys(properties)});
const choice=(...values)=>({type:'string',enum:values});
export const VIVADO_API_VERSION='vivado-native-plan-2';
...
export const VIVADO_LEGACY_SCHEMA=obj({version:{type:'string',const:'vivado-native-plan-1'},complete:{type:'boolean'},summary:str,
...
export const VIVADO_SCHEMA=obj({...VIVADO_LEGACY_SCHEMA.properties,version:{type:'string',const:VIVADO_API_VERSION},topTransactions:TOP_TRANSACTIONS_SCHEMA});
// Interface IDs are also supplied knowledge. They are closed by the schema,
// not inferred from a model-provided prefix or arbitrary command name.
const nativeKnowledgeIds=new Set(VIVADO_SCHEMA.properties.operations.items.properties.apiCall.anyOf.map(c=>c.properties.interfaceId.const));
...
export function validateVivadoProgram(p,{eventIds=[],currentInputIds=null,knowledgeIds=null,events=null,final=false}={}){
 ...
 const invalidate=project=>{
  const runs=[...objects.values()].filter(o=>o.type==='run'&&o.parentId===project.id);
  assert(!runs.some(r=>r.state==='launched'),'cannot change design while a run is active');
  project.designRevision++;
  for(const r of runs)if(r.state==='completed')r.state='stale';
 };
 ...
 for(const o of p.operations){
  ...
  if(knowledgeIds){const unknown=o.knowledgeIds.filter(id=>!knowledgeIds.includes(id)&&!nativeKnowledgeIds.has(id));assert(!unknown.length,'unknown knowledge ID: '+unknown.join(', '));}
  const c=o.apiCall,a=c.arguments, receiver=objects.get(c.receiverId);
  if(c.command==='create_project'){
   assert(c.receiverId===''&&!objects.size,'one isolated new project only');assert(safeName(a.name)&&/^[A-Za-z0-9][A-Za-z0-9-]{3,79}$/.test(a.part),'unsafe project name/part');register(c.resultId,{type:'project',name:a.name,part:a.part,properties:{},files:[],designRevision:0});
  }else{
   ...
   if(c.command==='set_property'){
    ...
    if(receiver.properties[a.name]!==a.value&&(a.name==='target_language'||a.name==='top'&&receiver.name==='sources_1'))invalidate(receiver.type==='project'?receiver:objects.get(receiver.parentId));
    ...
    const type=c.command==='get_filesets'?'fileset':'run';
    const existing=[...objects.values()].find(x=>x.parentId===receiver.id&&x.name===a.name);
    if(existing){
     // Re-reading the same native object into the SAME logical binding is
     // idempotent. Never re-register it: that would erase properties or reset
     // completed/stale run state and its input revision.
     assert(existing.type===type&&existing.id===c.resultId,'reuse existing query binding '+existing.id+'; cannot create an alias');
    }else register(c.resultId,{type,parentId:receiver.id,name:a.name,properties:{},state:'not_started'});
    ...
    if(a.fileset==='sources_1')invalidate(receiver);
    ...
    if(receiver.name==='impl_1')assert([...objects.values()].some(x=>x.type==='run'&&x.parentId===receiver.parentId&&x.name==='synth_1'&&x.state==='completed'&&x.inputRevision===objects.get(receiver.parentId).designRevision),'implementation requires observed completed synthesis for current design revision');
    receiver.state='launched';receiver.inputRevision=objects.get(receiver.parentId).designRevision;
...
export function mergeVivadoChunks(chunks){
 ...
 return {...chunks.at(-1),initialScene:chunks[0].initialScene,summary:chunks.map(c=>c.summary).join('; '),complete:chunks.every(c=>c.complete),artifacts:chunks.flatMap(c=>c.artifacts),operations:chunks.flatMap((c,i)=>c.operations.map((o,j)=>({...o,id:`chunk-${i+1}-op-${j+1}`}))),topTransactions:chunks.flatMap((c,i)=>c.topTransactions.map((t,j)=>({...t,id:`chunk-${i+1}-top-${j+1}`,operationId:t.operationId?`chunk-${i+1}-op-${c.operations.findIndex(o=>o.id===t.operationId)+1}`:''}))),decisions:chunks.flatMap(c=>c.decisions),unresolved:chunks.flatMap(c=>c.unresolved)};
}
...
export function vivadoPreviousContext(chunks,catalog){return chunks.length?{catalog,commandState:chunks.at(-1).commandState,summary:chunks.at(-1).summary,unresolved:chunks.flatMap(c=>c.unresolved),topTransactions:chunks.at(-1).topTransactions,pendingTopTransactions:pendingTopTransactions(chunks.flatMap(c=>c.topTransactions))}:null;}
