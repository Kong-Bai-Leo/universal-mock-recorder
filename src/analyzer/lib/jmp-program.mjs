// Versioned bounded operations plus MODEL-SUPPLIED native interface calls.
import {jmpApiCallSchema,validateJmpApiCalls,JMP_API_CATALOG_VERSIONS} from './jmp-api-contract.mjs';
...
export const JMP_SCHEMA=obj({
  version:{type:'string',const:'2.0'},apiCatalogVersion:{type:'string',enum:JMP_API_CATALOG_VERSIONS},complete:{...bool,description:JMP_COVERAGE_RULE},summary:str,
  initialScene:{...obj({kind:choice('blank','contains_existing','continuation','unknown'),evidenceIds:strs}),description:'Scope is the replay targets, not every open JMP document. blank means the tracked tables are created independently during this recording; an untouched background table does not change it. contains_existing means a preexisting table supplies data or is modified and is unsupported for final replay.'},
  finalScene:{...obj({kind:choice('observed','unknown'),tableIds:strs,reportIds:strs,evidenceIds:strs}),description:'Inventory only tracked replay targets with IDs established by committed create_table or analysis operations. Exclude untouched background documents and never invent an ID for one.'},
...
export function jmpSchemaFor(target){
  if(!JMP_API_CATALOG_VERSIONS.includes(target.apiCatalogVersion))throw Error('Unsupported JMP target catalog');
  return {...JMP_SCHEMA,properties:{...JMP_SCHEMA.properties,apiCatalogVersion:{type:'string',const:target.apiCatalogVersion}}};
}
...
export function validateJmpProgram(program,{eventIds=[],currentInputIds=null,knowledgeIds=null,final=false,apiCatalogVersion=null}={}){
  checkJmpSchema(program);
  if(apiCatalogVersion&&program.apiCatalogVersion!==apiCatalogVersion)throw Error('JMP API catalog does not match recording version');
  ...
  // A validated committed operation is already an explicit modeled decision.
  // Do not require a second copy of its source IDs in the explanatory ledger.
  // Unmodeled inputs still require a decision; scene/pending references alone
  // are not coverage, and all operation/API checks above remain mandatory.
  if(currentInputIds){
    const covered=new Set([...modeled,...program.decisions.flatMap(d=>d.sourceEventIds)]);
    const missing=currentInputIds.filter(id=>!covered.has(id));
    assert(!missing.length,'input coverage gap: '+missing.slice(0,20).join(', '));
  }
...
export function mergeJmpChunks(chunks){
  assert(chunks.length>0,'no chunks');chunks.forEach((c,i)=>{
    checkJmpSchema(c);
    assert(c.apiCatalogVersion===chunks[0].apiCatalogVersion,'JMP catalog changed between chunks');
...
export const JMP_INSTRUCTIONS=`Analyze the supplied JMP screenshots/input for the explicitly identified version and edition. Return version 2.0 structured JSON containing BOTH each observed operation AND the native interface call needed to reproduce it, never executable JSL.
${JMP_COVERAGE_RULE}
...
First establish whether the REPLAY TARGETS start blank: blank means every tracked table is created independently in this recording, even if unrelated empty or populated JMP documents were already open in the background. Do not model or assign IDs to untouched background documents. If an existing document supplies source data or is modified, it is a replay target with unseen initial state: opening/reusing it is unsupported, so report unresolved rather than inventing contents. Later chunks continue previous tracked tables, columns, reports and pending dialog state.
...
At final recording state establish complete final inventory of TRACKED replay tables/reports created by committed operations, using their legally established IDs. Exclude untouched background tables even if visible, and do not count log/help windows as reports. Never add an ID merely to mirror every open JMP window. Report finalScene.kind unknown if the tracked final state is obscured. Timestamps and delayed after-frame attribution matter. Entire source native table must NOT be requested/read to fill visual gaps. Summary concise; no statement that replay ran or matched.`;
export function jmpInstructionsFor(target){
  return `Target application: ${target.displayName}; UI language: ${target.language}; API catalog: ${target.apiCatalogVersion}.\n${JMP_INSTRUCTIONS}`;
}
