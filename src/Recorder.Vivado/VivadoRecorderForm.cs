...
namespace UniversalMockRecorder
{
    ...
    internal sealed class VivadoRecorderForm : Form
    {
        ...
        readonly Button openScript = new Button { Text = "打开脚本位置", Left = 20, Top = 205, Width = 185, Enabled = false };
        readonly Label status = new Label { Text = "就绪。Ctrl+Shift+F12 暂停/恢复。", Left = 20, Top = 245, Width = 560, Height = 110 };
        RecorderEngine engine;
        string recording;
        string replayScript;
        bool busy;
        public VivadoRecorderForm()
        {
            Text = RecorderProfile.WindowTitle; Width = 620; Height = 405;
            StartPosition = FormStartPosition.CenterScreen;
            Controls.Add(new Label { Text = "最终输出一个 .tcl 文件，直接在 Vivado 中运行，无需另交给 AI。\n必须在 Vivado 所在 VM 内录制全桌面截图，请关闭敏感窗口。", Left = 20, Top = 15, Width = 570, Height = 60 });
            Controls.AddRange(new Control[] { start, stop, pause, retry, prepare, analyze, folder, uia, openScript, status });
            start.Click += delegate {
                try {
                    ...
                    if (engine != null) engine.Dispose(); engine = new RecorderEngine(recording, uia.Checked); engine.Start(); RefreshControls(); WindowState = FormWindowState.Minimized;
        ...
        void RefreshControls()
        {
            ...
            prepare.Enabled = analyze.Enabled = retry.Enabled = folder.Enabled = uia.Enabled = !busy && !active;
        ...
        async Task Analyze(bool prepareOnly, bool retryFailed = false)
        {
            ...
            busy = true; replayScript = null; RefreshControls(); status.Text = prepareOnly ? "正在本地准备，不调用 API……" : "正在分析；完成后生成可直接运行的 Tcl，达到预算后保存检查点。";
            try {
                var root = Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", ".."));
                var info = new ProcessStartInfo("powershell.exe") { Arguments = "-NoProfile -ExecutionPolicy Bypass -File " + Quote(Path.Combine(root,"scripts","analyze-vivado-recording.ps1")) + " -Recording " + Quote(recording) + (prepareOnly ? " -PrepareOnly" : " -Analyze") + (retryFailed ? " -RetryFailed" : ""), WorkingDirectory = root, UseShellExecute = false, CreateNoWindow = true, RedirectStandardOutput = true, RedirectStandardError = true, StandardOutputEncoding = Encoding.UTF8, StandardErrorEncoding = Encoding.UTF8 };
        ...
        void ShowAnalysisResult(string output)
        {
            ...
            if (result.Status == "replay_script_generated_not_executed" || result.Status == "saved_replay_script_generated_not_executed") {
                if (string.IsNullOrEmpty(result.PrimaryOutput) || !string.Equals(Path.GetExtension(result.PrimaryOutput), ".tcl", StringComparison.OrdinalIgnoreCase) || !File.Exists(result.PrimaryOutput))
                    throw new IOException("分析完成，但未找到可运行的 Tcl 文件；请检查分析状态文件。");
                replayScript = result.PrimaryOutput;
                status.Text = "已生成 vivado-replay.tcl（尚未实机执行）。\n先关闭当前工程，在 Vivado 的 Tools > Run Tcl Script 中选择它。\n点击“打开脚本位置”；仅复制该文件即可，不需要 JSON 或 API 密钥。";
