...
namespace UniversalMockRecorder
{
    ...
    internal sealed class StataRecorderForm : Form
    {
        ...
        readonly CheckBox english = new CheckBox { Text = "我已在目标 Stata 界面确认语言为 English (en-US)", Left = 20, Top = 190, Width = 540 };
        ...
        string replayScript;
        bool busy;
        public StataRecorderForm()
        {
            Text = RecorderProfile.WindowTitle; Width = 620; Height = 425;
            StartPosition = FormStartPosition.CenterScreen;
            Controls.Add(new Label { Text = "最终输出一个 .do 文件，可在 Stata 中运行，无需另交给 AI。\n必须在 Stata 所在 VM 内录制全桌面截图，请关闭敏感窗口。", Left = 20, Top = 15, Width = 570, Height = 60 });
            Controls.AddRange(new Control[] { start, stop, pause, retry, prepare, analyze, folder, uia, english, openScript, status });
            start.Click += delegate {
                try {
                    if (!english.Checked) throw new InvalidOperationException("请先在 Stata 中确认界面语言，并勾选英文确认项。不会依据 Windows 语言推测。");
                    replayScript = null;
        ...
        async Task Analyze(bool prepareOnly, bool retryFailed = false)
        {
            ...
            busy = true; replayScript = null; RefreshControls(); status.Text = prepareOnly ? "正在本地准备，不调用 API……" : "正在分析；完成后生成可直接运行的 do，达到预算后保存检查点。";
            try {
                var root = Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", ".."));
                var info = new ProcessStartInfo("powershell.exe") { Arguments = "-NoProfile -ExecutionPolicy Bypass -File " + Quote(Path.Combine(root,"scripts","analyze-stata-recording.ps1")) + " -Recording " + Quote(recording) + (prepareOnly ? " -PrepareOnly" : " -Analyze") + (retryFailed ? " -RetryFailed" : ""), WorkingDirectory = root, UseShellExecute = false, CreateNoWindow = true, RedirectStandardOutput = true, RedirectStandardError = true, StandardOutputEncoding = Encoding.UTF8, StandardErrorEncoding = Encoding.UTF8 };
        ...
        void ShowAnalysisResult(string output)
        {
            ...
            if (result.Status == "replay_script_generated_not_executed" || result.Status == "saved_replay_script_generated_not_executed") {
                if (string.IsNullOrEmpty(result.PrimaryOutput) || !string.Equals(Path.GetExtension(result.PrimaryOutput), ".do", StringComparison.OrdinalIgnoreCase) || !File.Exists(result.PrimaryOutput))
                    throw new IOException("分析完成，但未找到可运行的 do 文件；请检查分析状态文件。");
                replayScript = result.PrimaryOutput;
                status.Text = "已生成 stata-replay.do（尚未实机执行）。\n在 Stata 脚本编辑器中打开，取消文本选择后点 Do。\n点击“打开脚本位置”；仅复制该文件即可，不需要 JSON 或 API 密钥。";
