...
import {JMP_SCHEMA,JMP_INSTRUCTIONS,validateJmpProgram,mergeJmpChunks,jmpChunkPosition,jmpPreviousContext} from '../src/analyzer/lib/jmp-program.mjs';
...
test('JMP UI Map hierarchy and no-Automation-ID retrieval remain available',async()=>{
  const k=await loadJmpKnowledge(path.resolve('.'));assert.ok(k.controls.length>40);const r=retrieveJmpKnowledge(k,[{key:'ENTER'}],null);assert.ok(r.controls.some(c=>c.id==='jmp.table.modeling-type'));assert.ok(r.controls.some(c=>c.label==='Fit Y by X'));assert.ok(r.controls.every(c=>!c.automationId));
...
async function fixture(t,count=1){
  ...
  await fs.writeFile(path.join(dir,'manifest.json'),JSON.stringify({applicationProfile:'jmp',captureDeployment:'same-windows-session',uiAutomationTargets:false}));
