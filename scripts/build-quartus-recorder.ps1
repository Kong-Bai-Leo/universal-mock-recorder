...
$outputDirectory = Join-Path $workspaceRoot 'bin\quartus-recorder'
$outputFile = Join-Path $outputDirectory 'QuartusRecorder.exe'
...
& $compiler /nologo /target:winexe /optimize+ /platform:x64 /define:QUARTUS `
    /out:$outputFile `
...
if ($LASTEXITCODE -ne 0) {
    throw "QUARTUS recorder build failed with exit code: $LASTEXITCODE"
}

Write-Output "QUARTUS recorder built: $outputFile"
