...
$outputDirectory = Join-Path $workspaceRoot 'bin\3dsmax-recorder'
$outputFile = Join-Path $outputDirectory 'ThreeDsMaxRecorder.exe'
...
& $compiler /nologo /target:winexe /optimize+ /platform:x64 /define:THREEDSMAX `
    /out:$outputFile `
...
if ($LASTEXITCODE -ne 0) {
    throw "3ds Max recorder build failed with exit code: $LASTEXITCODE"
}

Write-Output "3ds Max recorder built: $outputFile"
