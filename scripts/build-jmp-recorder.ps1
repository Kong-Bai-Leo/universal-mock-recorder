...
$recorderOutput = Join-Path $recorderRoot 'bin\jmp-recorder'
...
& $recorderCompiler /nologo /target:winexe /optimize+ /platform:x64 /define:JMP `
    ("/out:" + (Join-Path $recorderOutput 'JmpRecorder.exe')) `
    ...
    (Join-Path $recorderRoot 'src\Recorder.Jmp\JmpRecorderForm.cs')
if ($LASTEXITCODE -ne 0) { throw 'JMP recorder build failed.' }
Write-Output (Join-Path $recorderOutput 'JmpRecorder.exe')
