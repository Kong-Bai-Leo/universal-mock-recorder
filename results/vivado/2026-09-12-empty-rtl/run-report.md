...
验证阶段的最新回归为 80/80（Vivado 27、JMP 53）；本次发布没有重新运行模型或实机回放。录制、截图、请求体留本地并被 Git 忽略。

## 运行结果发布补充

2026-09-12 用户明确要求将运行结果 push。发布范围是本目录的接口 JSON、原执行 Tcl、原生 XPR、调用日志、通过标记、独立源属性和观察对比报告，不包含原始录制、截图、API 请求/响应、密钥或其他待提交的开发代码。

实际 XPR、calls.log 和 verification-status.txt 经本次 VM 文件复制取回；原生缓存和硬件窗口布局未发布。JSON 计划、Tcl 与 XPR 未改写；观察报告只调整了发布副本的真值相对路径。哈希见同目录 manifest.json。发布校验仅检查文件完整性、格式和已知属性，不能替代上文的实机验证。
